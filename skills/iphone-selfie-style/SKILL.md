...
- **Nano Banana Pro needs more explicit/redundant instruction than GPT Image 2 to land the same
  framing on identical prompt text.** A side-by-side test with the exact same prompt on both
  models found GPT-2 nailed the arm-blocks-the-phone framing natively (her arm filling most of
  the foreground, soft-focus from proximity, naturally cropping the hand/phone out of frame) while
  Nano Banana Pro did not — on the same words. GPT-2 stayed fully photoreal and had weaker facial
  fidelity though, so it's not a model swap, just a reminder that Nano Banana needs the "no
  visible phone" constraint spelled out more concretely/redundantly than a more instruction-
  faithful model would require. Describe the arm (sleeved or bare, depending on the outfit) as
  filling a large part of the foreground, soft and slightly out of focus from proximity to the
  lens — this gives the "phone never visible" rule a physical/optical reason, not just a bare
  negation.
