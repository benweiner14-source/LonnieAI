...
- **Attach one photo** — `creators/selena/refs/selena_gaming_room_pink_chair.png` or
  ...
  stronger face refs than the original `selena_car_daylight_portrait.png` — Higgsfield only
  reliably uses one attached image, so pick whichever of the two reads as the stronger likeness
  for a given seed), or `selena_black_sweats_mirror.png` if you want her figure/silhouette to
...
place names as legible signage, and no real automaker logos on vehicles — both now baked into
every scene's paste-ready text below**, ported over from the two confirmed leaks found on the
Comfy/Nano Banana Pro pack: the `nightlife` validation render rendered "Ocean Drive" (a real
Miami street) as readable signage unprompted, and the `luxury-car`/`club-entrance` validation
batch rendered a real Ford Mustang grille badge and an Audi rings badge unprompted. Not yet
re-tested on this Higgsfield pack specifically — same underlying model family, so treat the fix
as carried-forward-but-unconfirmed-here until a Higgsfield render is checked directly.
